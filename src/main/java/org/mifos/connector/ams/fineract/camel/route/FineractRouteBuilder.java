...
public class FineractRouteBuilder extends RouteBuilder {
  ...
  public void configure() throws Exception {
        ...
        .process(
            exchange -> {
              ...
              exchange.setProperty(
                  ACCT_HOLDING_INSTITUTION_ID_VARIABLE_NAME,
              ...
              exchange.setProperty(
                  AMOUNT_VARIABLE_NAME, exchange.getProperty(AMOUNT_VARIABLE_NAME));
              exchange.setProperty(
                  CURRENCY_VARIABLE_NAME, exchange.getProperty(CURRENCY_VARIABLE_NAME));
              exchange.setProperty(
                  MSISDN_VARIABLE_NAME, exchange.getProperty(MSISDN_VARIABLE_NAME));
            ...
            exchange -> {
              ...
              if (exchange.getProperty(CHANNEL_REQUEST) != null) {
                ...
                verificationRequestDTO =
                    FineractRequestDto.fromChannelRequest(
                        channelRequest, exchange.getProperty(TRANSACTION_ID, String.class));
              } else {
                JSONObject payBillRequest = new JSONObject(exchange.getIn().getBody(String.class));

                verificationRequestDTO =
                    FineractRequestDto.convertPayBillPayloadToAmsPayload(payBillRequest);
                exchange.setProperty(
                    TRANSACTION_ID, verificationRequestDTO.getRemoteTransactionId());
    ...
    from("direct:transfer-settlement-base")
        ...
        .choice()
        .when(header(CAMEL_HTTP_RESPONSE_CODE).isEqualTo("200"))
        .log(LoggingLevel.INFO, "Call to Fineract AMS for settlement was successful")
        .process(
            exchange -> {
              // processing success case
        ...
        .otherwise()
        .log(LoggingLevel.ERROR, "Call to  Fineract AMS for settlement was unsuccessful")
        .process(
            exchange ->
                // processing unsuccessful case
            ...
            exchange -> {
              ...
              if (exchange.getProperty(CHANNEL_REQUEST) != null) {
                ...
                confirmationRequestDto =
                    FineractConfirmationRequestDto.fromChannelRequest(
                        channelRequest, transactionId);
                ...
                confirmationRequestDto =
                    FineractConfirmationRequestDto.convertPayBillPayloadToAmsPayload(
                        payBillRequest);
                exchange.setProperty(
                    TRANSACTION_ID, confirmationRequestDto.getRemoteTransactionId());
              }
